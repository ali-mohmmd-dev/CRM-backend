...
def reset_producer() -> None:
    """Clear the cached producer (useful in tests)."""
    global _producer
    _producer = None
...
def publish(
    ...
    *,
    ...
    """
    Publish a domain event envelope to Kafka.

    topic_key must be a key in settings.KAFKA_TOPICS (e.g. 'leads').
    Partition key defaults to organization_id for per-org ordering.
    """
